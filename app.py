...
# v3 FIX12  (CRUD + righe rosse + salva rapido + robust table)
...
st.set_page_config(
    page_title="CRM Clienti & Contratti — v3 FIX12",
    layout="wide"
)
...
def fmt_date(d):
    """Ritorna stringa dd/mm/yyyy (senza ora)."""
...
def numify(x):
    """Converte valuta/stringhe in float standard (punto come decimale)."""
...
def euro(x):
    """Formatta float in € con virgola italiana; vuoto se 0."""
    v = numify(x)
    if v == 0:
        return ""
...
def load_data():
    ...
    # CONTRATTI
    ctr_cols = ["ClienteID"] + SAFE_CONTRACT_COLS
    ...
    # normalizzazioni
    contratti = sanitize_contracts_df(contratti)
    # ricalcolo TotRata dove necessario
    ...
    # ricostruisci con ClienteID
    contratti = pd.concat([contratti, contratti[[]]], axis=1)
    contratti.insert(0, "ClienteID", pd.to_numeric(contratti.get("ClienteID", pd.Series(dtype="Int64")), errors="coerce").astype("Int64"))
    # assicurati dell'ordine
    contratti = contratti[["ClienteID"] + SAFE_CONTRACT_COLS]
...
# =========================
# Tabella contratti (HTML, righe rosse)
# =========================
...
def render_clienti():
        ...
        if target:
            ...
            with st.form("form_edit_ctr"):
                ...
                if ok:
                    ...
                    st.session_state["contratti"].loc[
                        mask,
    ...
    # ---- Salvataggio rapido (pagina Clienti)
    c1, c2 = st.columns(2)
...
def render_settings():
    ...
    if ut is not None and editable:
        tmp = pd.read_csv(ut)
        tmp = sanitize_contracts_df(tmp)
        calc = tmp.apply(compute_tot, axis=1)
