...
st.set_page_config(page_title="CRM Clienti & Contratti — v3 FIX7", layout="wide")
...
def render_settings():
    ...
    if ut is not None and role != "Viewer":
        ...
        tmp["DataInizio"] = tmp["DataInizio"].apply(fmt_date) if "DataInizio" in tmp.columns else ""
        tmp["DataFine"] = tmp["DataFine"].apply(fmt_date) if "DataFine" in tmp.columns else ""
        for col in ["NOL_FIN","NOL_INT","TotRata"]:
        ...
        if "NOL_FIN" in tmp.columns and "NOL_INT" in tmp.columns:
            ...
            if "TotRata" in tmp.columns:
                ...
                tmp["TotRata"] = calc
        tmp["Stato"] = tmp["Stato"].astype(str).replace({"nan":""}) if "Stato" in tmp.columns else ""
