...
st.set_page_config(page_title="CRM Clienti & Contratti — v3 FIX", layout="wide")
...
def status_class(s):
    # Robust: cast to string to avoid AttributeError on numbers/NaN
...
def load_data():
    ...
    cli_cols = EXPECTED_CLIENTI_COLS.copy()
...
# =========================
# Attachments storage (session; for persistenza reale usare DB/object storage)
# =========================
...
def sidebar(role):
    ...
    st.sidebar.caption("v3 • FIX • validazioni, allegati, preventivi, Excel/print")
...
def render_dashboard(clienti, contratti):
    # HOTFIX: make sure expected cols exist
    clienti = ensure_clienti_cols(clienti)
            ...
            if tgt.empty:
                ...
                with st.form("edit_client"):
                    ...
                    if save_btn:
                        ...
                        if ur and not ur_d: errs.append("Formato data Ultimo Recall non valido (dd/mm/aaaa).")
                        if pr and not pr_d: errs.append("Formato data Prossimo Recall non valido (dd/mm/aaaa).")
                        if uv and not uv_d: errs.append("Formato data Ultima Visita non valida (dd/mm/aaaa).")
                        if pv and not pv_d: errs.append("Formato data Prossima Visita non valida (dd/mm/aaaa).")
                        if errs:
                            for e in errs: st.toast(e, icon="⚠️")
                        else:
...
""", unsafe_allow_html=True)
            ...
            # Excel download con fallback a CSV se manca XlsxWriter
            try:
                out = BytesIO()
            ...
            # Print-friendly HTML con nome cliente centrato
            html_rows = "".join([
            ...
            if templates:
                ...
                if gen:
                    ...
                    try:
                        ...
                        st.toast("Preventivo generato. (Per aprirlo in Word scarica il file.)", icon="✅")
