...
def page_dashboard(df_cli, df_ct, role):
    ...
    scadenza = df_ct[(df_ct["DataFine"].notna()) & (df_ct["DataFine"] <= now + pd.DateOffset(months=6)) & (df_ct["Stato"].str.lower() != "chiuso")]
    if scadenza.empty:
    ...
    with col_r:
        ...
        st.dataframe(df_cli[["RagioneSociale", "UltimoRecall", "ProssimoRecall"]].sort_values("UltimoRecall", ascending=False).head(5))
    with col_v:
        st.markdown("#### 🚗 Ultime Visite")
        st.dataframe(df_cli[["RagioneSociale", "UltimaVisita", "ProssimaVisita"]].sort_values("UltimaVisita", ascending=False).head(5))
# ==========================
# CLIENTI
# ==========================
