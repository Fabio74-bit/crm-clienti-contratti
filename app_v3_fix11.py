...
st.set_page_config(page_title="CRM Clienti & Contratti — v3 FIX11 (CRUD + righe rosse + salva rapido)", layout="wide")
...
# =========================
# Table rendering (rows red if closed) + save buttons
# =========================
def contracts_html(df):
    df = df.copy().fillna("")
    df["NOL_FIN_f"] = df["NOL_FIN"].apply(euro)
    df["NOL_INT_f"] = df["NOL_INT"].apply(euro)
    df["TotRata_f"] = df["TotRata"].apply(euro)
    order = ["NumeroContratto","DataInizio","DataFine","Durata","DescrizioneProdotto",
             "NOL_FIN_f","NOL_INT_f","TotRata_f","Stato"]
    df = df[order].rename(columns={"NOL_FIN_f":"NOL_FIN","NOL_INT_f":"NOL_INT","TotRata_f":"TotRata"})
    head = '''
    <style>
    ...
    <table class="ctr-table">
      <thead><tr>{}</tr></thead>
      <tbody>
    '''.format("".join([f"<th>{c}</th>" for c in df.columns]))
    rows = []
...
# =========================
# Clienti (CRUD) + Contratti (CRUD) in scheda cliente
# =========================
def render_clienti():
    ...
    with st.expander("🗑️ Elimina cliente", expanded=False):
        ...
        if st.button("Elimina definitivamente", disabled=(not editable or del_id is None)):
            ...
            # rimuovi anche i suoi contratti
    ...
    # Tabella contratti (righe rosse per chiusi)
    ...
    # ---- Save buttons (rapidi) in pagina Clienti
    c1, c2 = st.columns(2)
...
# =========================
# Contratti per cliente (vista + salva rapido)
# =========================
